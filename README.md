...
Класс `WebLarekApi` представляет собой реализацию API для взаимодействия с сервером веб-приложения. Он наследуется от базового класса `Api` и предоставляет методы для загрузки данных о продуктах, получения информации о конкретном продукте и размещения заказов. Этот класс реализует интерфейс `IWebLarekApi`, что обеспечивает строгую типизацию и четкое описание доступных методов.

`constructor(cdn: string, baseUrl: string, options?: RequestInit)` Конструктор принимает:

- `cdn`: строку с базовым URL CDN для изображений.
- `baseUrl`: базовый URL API для выполнения запросов.
- `options`: дополнительные параметры для HTTP-запросов (опционально).
  Внутри конструктора:
- Инициализируется базовый класс `Api` с переданными параметрами (`baseUrl`, `options`).
- Сохраняется ссылка на CDN для использования при обработке путей изображений.

#### Методы класса

- `getProducts(): Promise<IProductList>` Загружает список продуктов с сервера:
  - Выполняет GET-запрос к эндпоинту `/product`.
  - Добавляет CDN к путям изображений каждого продукта, заменяя расширение `.svg` на `.png`.
  - Возвращает объект типа `IProductList` с обновленными данными.
- `getProductById(id: string): Promise<IProduct | IProductNotFound>` Загружает информацию о конкретном продукте по его ID:
  - Выполняет GET-запрос к эндпоинту `/product/${id}`.
  - Если сервер возвращает ошибку, возвращается объект типа `IProductNotFound`.
  - В противном случае добавляется CDN к пути изображения продукта, и возвращается объект типа `IProduct`.
- `placeOrder(order: IOrder): Promise<IOrderSuccess | IOrderError>` Размещает заказ на сервере:
  - Выполняет POST-запрос к эндпоинту `/order` с данными заказа.
  - Если сервер возвращает ошибку, возвращается объект типа `IOrderError`.
  - В противном случае возвращается объект типа `IOrderSuccess`.

#### Интерфейсы, связанные с классом

- `IWebLarekApi` Интерфейс описывает методы, которые должны быть реализованы классом:
  - `getProducts: () => Promise<IProductList>` загружает список продуктов.
  - `getProductById: (id: string) => Promise<IProduct | IProductNotFound>` загружает информацию о продукте по ID.
  - `placeOrder: (order: IOrder) => Promise<IOrderSuccess | IOrderError>` размещает заказ.
- `IProductList` Интерфейс описывает структуру списка продуктов. Содержит следующие поля:
...
- `IProduct` Интерфейс описывает структуру продукта. Содержит следующие поля:
  ...
  - `category: string` — категория продукта.
  - `price: number | null` — цена продукта (может быть `null`, если цена не указана).
- `IProductNotFound` Интерфейс описывает структуру ошибки при отсутствии продукта. Содержит следующие поля:
...
- `IOrder` Интерфейс описывает структуру заказа. Содержит следующие поля:
  ...
  - `total: number` — общая стоимость заказа.
  - `items: string[]` — массив ID продуктов в заказе.
- `IOrderSuccess` Интерфейс описывает успешный результат размещения заказа. Содержит следующие поля:
  - `id: string` — уникальный идентификатор заказа.
  - `total: number` — общая стоимость заказа.
- `IOrderError` Интерфейс описывает ошибку при размещении заказа. Содержит следующие поля:
  - `error: string` — сообщение об ошибке.

Класс `WebLarekApi` предоставляет удобный и надежный способ взаимодействия с сервером. Его методы позволяют легко загружать данные о продуктах, получать информацию о конкретном продукте и размещать заказы, обрабатывая как успешные результаты, так и ошибки. Наследование от `Api` делает его расширяемым и совместимым с общей архитектурой приложения.
