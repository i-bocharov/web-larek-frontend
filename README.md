...
- Инициализирует элементы интерфейса:
  - Заголовок товара (`_title`)
  - Цена товара (`_price`)
  - Категория товара (`_category`)
  - Изображение товара (`_image`)
  - Описание товара (`_description`)
  - Корневой элемент для клика (`_button`)
...
- `_description: HTMLElement | null` - элемент описания товара.
- `_button: HTMLElement` - корневой элемент карточки для клика.

#### Методы класса

- `private get isFree(): boolean` - возвращает true, если товар бесплатный (по тексту цены).
- `set id(value: string)` / `get id(): string` - геттер и сеттер для идентификатора товара (через dataset).
- `set title(value: string)` - устанавливает текст заголовка.
- `set price(value: number | null)` - устанавливает цену товара, обновляет состояние кнопки ("В корзину" или disabled для бесплатных).
...
- `render(data: IProduct): HTMLElement` - основной метод для отображения карточки:
  - Устанавливает все поля карточки на основе переданных данных.
...
Класс `Card` предоставляет универсальный и расширяемый способ отображения карточки товара, управления её состоянием и интеграции с системой событий для взаимодействия с другими компонентами приложения. Его методы позволяют гибко настраивать внешний вид карточки и обеспечивать корректную работу пользовательских сценариев (добавление в корзину, предпросмотр и т.д.).
...
Класс `Preview` реализует компонент предпросмотра товара в модальном окне. Он наследуется от базового класса `Component<IProduct>` и предоставляет методы для отображения информации о выбранном товаре, управления состоянием кнопки и интеграции с системой событий через интерфейс `IEvents`.

#### Конструктор

`constructor(container: HTMLElement, events: IEvents)`

- Принимает контейнер предпросмотра (`container`) и экземпляр системы событий (`events`).
- Инициализирует элементы интерфейса:
  - Заголовок товара (`_title`)
  - Изображение товара (`_image`)
  - Описание товара (`_description`)
...
- `set id(value: string)` / `get id(): string` - геттер и сеттер для идентификатора товара (через dataset).
- `set title(value: string)` - устанавливает текст заголовка.
- `set price(value: number | null)` - устанавливает цену товара, обновляет состояние кнопки ("Бесплатно" или "В корзину").
- `set category(value: string)` - устанавливает текст и класс категории.
- `set image(value: string)` - устанавливает изображение товара.
- `set description(value: string)` - устанавливает описание товара.
- `render(data: IProduct): HTMLElement` - основной метод для отображения предпросмотра:
  - Устанавливает все поля на основе переданных данных.
  - Возвращает контейнер предпросмотра.

#### Типы, связанные с классом

- `IProduct` - интерфейс товара.
- `IEvents` - интерфейс системы событий, используется для генерации событий:
  - `preview:button-click` - при нажатии на кнопку действия в предпросмотре.

#### Итог

Класс `Preview` предоставляет удобный и расширяемый способ отображения информации о выбранном товаре в модальном окне. Его методы позволяют гибко управлять внешним видом предпросмотра, состоянием кнопки и интеграцией с системой событий для передачи данных другим компонентам приложения.
