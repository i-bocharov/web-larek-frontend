...
`constructor(container: HTMLElement, protected events: EventEmitter)` Конструктор принимает контейнер корзины (`container`) и экземпляр системы событий (`events`). Внутри конструктора инициализируются элементы интерфейса (список товаров, поле общей стоимости и кнопка оформления заказа). Также на кнопку оформления заказа добавляется обработчик события, который генерирует событие `order:open`. Клик по кнопке удаления товара генерирует событие `basket:item-removed`.
